...
*(Figure 2. Synthetic image samples generated with TINTOlib using the TINTO method, one representative instance for each class of the multiclass dataset. Each image encodes the feature values of a tabular sample into a two-dimensional spatial representation, where the arrangement and intensity patterns reflect class-dependent feature distributions that can be exploited by computer vision and hybrid neural architectures.)*
